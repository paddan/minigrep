...
`benchmark.sh` runs each query 10 times (+3 warmup) against `mg` and `ripgrep`, verifies match counts agree, and emits a markdown table. It also synthesizes a ~800MB large file by concatenating `weather_stations.csv` 1000× to `mktemp`. Both `bc` and `python3` (for sub-second timing) are required. ripgrep is invoked with `-j <core count>` for fairness, though this measurably makes no difference — rg only parallelizes across files in a directory walk, not within a single file. The `Winner` column picks the fastest of the two, within a ±2% tie band.

`run_test_suite` (in `benchmark.sh`) holds the single list of test cases — exact, case-insensitive, and regex — and is called once for the standard file and once for the synthesized large file, so both sections compare identical queries. The regex cases cover anchors/quantifiers (`^[A-Z][a-z]+;`), escaping (`;-?[0-9]+\.`), alternation (`^(San|New|Los) `), a second literal class run (hyphenated city names), bounded quantifiers combined with an end anchor (`;-?[0-9]{1,2}\.[0-9]{4}$`), and combined regex+case-insensitive (`-ri '^[a-z]'`).
